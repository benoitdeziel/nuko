...
const defaultSchema: NestApplicationGeneratorSchema = {
...
describe('Application generate generator', () => {
  ...
  it('should generate application and default libraries', async () => {
    ...
    expect(
      tree.exists(
        `apps/api/${defaultSchema.applicationName}/src/migrations.ts`,
      ),
    ).toBeTruthy();

    expect(
      tree.exists(
        `apps/api/${defaultSchema.applicationName}/src/app.module.ts`,
      ),
    ).toBeTruthy();
