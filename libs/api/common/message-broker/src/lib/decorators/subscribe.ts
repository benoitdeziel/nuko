...
export function Subscribe(
  ...
  const exchange =
    options.exchange ?? MessageBrokerExchangeUtil.getDefaultExchange();
  ...
  const queueOptions: QueueOptions = {
    ...
    arguments: {
      'x-dead-letter-exchange':
        MessageBrokerExchangeUtil.getDeadLetterExchangeName(exchange.name),
  ...
  const decorators: MethodDecorator[] = [
    ...
    SetMetadata(RABBIT_RETRY_HANDLER, {
      ...
      exchange: MessageBrokerExchangeUtil.getRetryExchangeName(exchange.name),
      ...baseMessageHandlerOptions,
    }),
  ];
...
function createErrorHandler(
  ...
  return (channel: Channel, msg: ConsumeMessage, error) => {
    ...
    if (retryAttempt < requeueOnError.maxRetries) {
      ...
      channel.publish(
        MessageBrokerExchangeUtil.getRetryExchangeName(exchangeName),
